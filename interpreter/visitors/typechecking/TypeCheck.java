...
import static interpreter.visitors.typechecking.PrimType.*;
...
public class TypeCheck implements Visitor<Type> {
	...
	//// operatore !=
	@Override
	public PrimType visitNot_Eq(Exp left, Exp right){
		left.accept(this).checkEqual(right.accept(this));
		return BOOL;
	}
	...
	/// Literal Range
	@Override
	public Type visitRange(Exp left, Exp right) {
		return  new RangeType(left.accept(this), right.accept(this));
	}

	// operatore bounds
	@Override
	public Type visitBounds(Exp exp) {
		exp.accept(this).checkIsRangeType();
		var start = exp.accept(this).getStartRangeType();
		var end = exp.accept(this).getEndRangeType();
		return new ProdType(start, end);
	}

	//Stmt for
	@Override
	public Type visitFor(VarIdent ident, Exp exp, Block block) {
				exp.accept(this).checkIsRangeType();
				env.enterScope();
				env.dec(ident, INT);
				block.accept(this);
